function [ bugCoords ] = getBugCoords2( Dtrain, sections )
%bugCoords returns a matrix of size N x 3 where each row of the matrix
%represents the (x, y, z) coordinates of pixels that the user labelled as
%bugs.
dim1 = size(Dtrain,1);
dim2 = size(Dtrain,2);
dim3 = size(Dtrain,3);
...
for j = 1:sections
    for k = 1:sections
        for i = 1:dim3
            ...
            for u = 1:size(x,1)
                tempBugCoords = [tempBugCoords; x(u) y(u) i];
            end
